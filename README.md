...
Cloud Code Light 是一个 macOS 菜单栏和悬浮交通灯工具，用颜色提示 Claude Code 当前状态：正在执行、等待你处理、已完成或空闲。

## 功能概览

- 菜单栏状态灯：常驻 macOS 菜单栏，方便快速查看状态。
- 悬浮交通灯：在桌面上显示更醒目的状态提示。
- Claude Code Hooks 集成：根据 Claude Code 的 hook 事件自动更新任务状态。
- 命令行控制：可手动设置状态、查询状态、清空状态或退出应用。
- 多任务聚合：多个 Claude Code 任务同时存在时，按优先级显示最需要关注的状态。
- 剩余额度展示：浮窗底部可显示 5 小时和 1 周剩余额度百分比。

## 状态说明

| 颜色 | 状态 | 含义 | 提示行为 |
| --- | --- | --- | --- |
| 黄灯 | `working` | Claude Code 正在工作 | 静默显示 |
...
双击运行安装脚本，自动完成构建、打包 .app、安装到 Applications、配置 Claude Code hooks：

```bash
./install.command
```

安装后可以从 Launchpad 或 Spotlight 启动 `CloudCodeLight`，也可以拖到 Dock 方便访问。

## 手动打包 .app

如果只想打包成 .app 而不配置 hooks：

```bash
./create-app-bundle.sh
```

产物位于 `~/Applications/CloudCodeLight.app`。

## 接入 Claude Code Hooks

本工具通过 Claude Code 的 hooks 机制自动检测 VS Code 中 Claude Code 的工作状态。hooks 同时对 Claude Code CLI 和 VS Code 扩展生效。

### 自动配置（推荐）

运行 `./install.command` 会自动将 hooks 写入 `~/.claude/settings.json`。

### 手动配置

将 `Docs/hooks-claude-code.example.json` 中的 hooks 配置合并到 `~/.claude/settings.json`。首次触发 hook 时，Claude Code 会提示信任该命令，选择 **Always allow**。
